...
const DoctorProfile = () => {
  ...
  const updateProfile = async () => {
    try {
      ...
      const {data} = await axios.post(`${backendUrl}/api/doctor/updateProfile`, {
        updateData,
        headers: {
          dToken
        }
    ...
    <div>
      ...
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
          ...
          {
            isEdit? <button onClick={updateProfile} className='px-4 py-1 border border-[#5F6FFF] text-sm rounded-full mt-5 hover:bg-[#5F6FFF] transition-all'>Save</button> :
            <button onClick={()=> setIsEdit(true)} className='px-4 py-1 border border-[#5F6FFF] text-sm rounded-full mt-5 hover:bg-[#5F6FFF] transition-all'>Edit</button>
          }
