...
const MyProfile = () => {
 ...
 const {backendURL, userData, setUserData, token, getUserData} = useContext(AppContext)
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        </label>:<img className='w-36 rounded' src={userData.image} alt={userData.name} />
      }
      {
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          {isEdit ? <select className='max-w-20 bg-gray-100' name="gender" id="gender" value={userData.gender} onChange={(e) => setUserData(prev =>({...prev, gender: e.target.value }))}>
      ...
      <div className='mt-10'>
        {
          isEdit ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all duration-100' onClick={updateUserProfileData}>Save Changes</button> : <button className='border border-primary px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all duration-100' onClick={() => setIsEdit(true)}>Edit Profile</button>
        }
      </div>
    </div>
  )
